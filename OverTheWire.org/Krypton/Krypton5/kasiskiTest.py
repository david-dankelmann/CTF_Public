...
#Find most occuring sequences
highest_sequence_count = 0
possible_combinations = list(itertools.product(letters, repeat = 3)) #specify length of sequences here.
sequences = {}
for combination in possible_combinations:
    combination = "".join(combination)
    occurence = ciphertext.count(combination)
    if(occurence > highest_sequence_count): #only keep most occuring sequences - should be enough
        sequences.update({combination : occurence})
        print(f"Sequence {combination} occurs {occurence} times.")
        highest_sequence_count = occurence
    


#Kasiski Test. We assume, that the sequences we found earlier are representing the same plaintext words. If they are, there distance in the text modulo key_length must be the same.
for key_length in range(3, max_key_length): #for each possible key length (limited by max_key_length)
    matches_counter = 0
    for combination, occurence in sequences.items(): #for every subsequence we found
        splitted_ciphertext = ciphertext.split(combination) 
        last_distance = 0
        for section in splitted_ciphertext:
            distance = len(section) % key_length
            if(distance == last_distance):
                matches_counter += 1


            last_distance = distance
            #print(f"Length of section is: {len(section)}")
    print(f"Distance matches with a keylength of {key_length}: {matches_counter}")
